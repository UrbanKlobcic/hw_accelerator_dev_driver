...
static int write_buf(int fd, uint32_t buf_id, const int32_t *vals, uint32_t num_elems)
{
    ...
    if (write(fd, &b, sizeof(b)) != (ssize_t)sizeof(b))
        return -1;
...
static void *thread_fn(void *arg)
{
    ...
    fd = open("/dev/dma_mtx_mul", O_RDWR);
    if (fd < 0) {
        perror("open");
        st->errors++;
        return NULL;
    ...
    while (!stop_flag) {
        ...
        if (out.num_elems != 4 ||
            memcmp(out.data, gold, 4 * sizeof(int32_t)) != 0) {
            fprintf(stderr,
                    "thread %d: result mismatch: "
                    "[%d %d; %d %d] != [%d %d; %d %d]\n",
                    st->id,
...
int main(void)
{
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        printf("thread %d: ops=%lu errors=%lu\n",
               i, stats[i].ops, stats[i].errors);
        total_ops += stats[i].ops;
        total_errors += stats[i].errors;
    }

    printf("TOTAL: ops=%lu errors=%lu\n", total_ops, total_errors);
