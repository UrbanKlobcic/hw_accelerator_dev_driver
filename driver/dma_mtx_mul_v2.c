...
#include "../include/dma_mtx_mul_ioctl_v2.h"
...
static int dma_mtx_mul_worker_fn(void *data)
{
    ...
    while (!kthread_should_stop()) {
        ...
        push_ret = dma_mtx_mul_desc_q_push(&owner->done_q, &d);
        if (push_ret)
        ...
        dev->regs.control &= ~DMA_MTX_MUL_CTRL_START;
        if (!dev->pending_q.count && dma_mtx_mul_desc_q_is_empty(&owner->done_q))
            dma_mtx_mul_reset_regs(dev);
...
static long dma_mtx_mul_handle_dequeue_done(struct file *file, unsigned long arg)
{
    ...
    if (!g_dma_mtx_mul_dev.worker_busy &&
        ...
        dma_mtx_mul_desc_q_is_empty(&ctx->done_q))
...
static long dma_mtx_mul_handle_get_buf_info(struct file *file, unsigned long arg)
{
    ...
    info.refcnt = g_dma_mtx_mul_dev.bufs[info.buf_id].refcnt;
...
MODULE_DESCRIPTION("Matrix multiplier accelerator driver with per-open isolation and safer lifetime handling.");
MODULE_VERSION("2.0");